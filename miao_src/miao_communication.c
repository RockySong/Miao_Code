/*(Ver.=0.96)
 * MIAO_communication.c
 *
 *  Created on: 2017-7-26
 *      Author: slw
 */

#include "OSLW_include.h"


#if MIAO_TASK_COMMUN_ENABLE

MiaoTask_t MiaoInlineTaskCommun,*pta_Commun=&MiaoInlineTaskCommun;


MiaoGiftUnit_t *qgu_Commun;
MiaoGiftReceive_t *pgr_Commun;
//MiaoModBus_t *qc_Commun;
//MiaoParaCmd_t *pcmd_Commun;
lw_u8 *arr_Commun;
lw_8 id_Commun,n_Commun;


void MiaoInlineTaskCommunExe(MiaoTask_t *pta_)
{

//	pgr_Commun=&(pta_Commun->Concierge.giftR);
//	qc_Commun=& (pta_Commun->pOS->Comm[0]);
//
//	MIAO_TASK_LOOP(pta_Commun)//循环开始
//	{
//		MiaoGiftReceiveWait(pta_Commun);
//		do
//		{
//			qgu_Commun=pgr_Commun->pHead->pGU;
//			qc_Commun->pTxHead=qc_Commun->pTxTail;
//			arr_Commun=qgu_Commun->PrivateUnit.ComCmd.RxD;
//			if(arr_Commun[0]==qc_Commun->Address && ((id_Commun=MiaoGiftTransmitIdleIndex(&pta_Commun->Concierge.giftT))!=(lw_8)-1))
//			{
//				qc_Commun=&(pta_Commun->pOS->Comm[qgu_Commun->PrivateUnit.ComCmd.ComID]);
//				*(qc_Commun->pTxTail)++=qc_Commun->Address;
//				switch(arr_Commun[1])
//				{

//				case 3:
//					*(qc_Commun->pTxTail)++=3;
//					*(qc_Commun->pTxTail)++=(MODBUS_CONNECT_UINT8_TO_UINT16(arr_Commun[4],arr_Commun[5]))*MIAO_PARA_NORMAL_LENGTH;

//					pcmd_Commun=&(pta_Commun->Concierge.GiftList[id_Commun].PrivateUnit.ParaCmd);

//					pcmd_Commun->ListID=1;
//					pcmd_Commun->ID=(MODBUS_CONNECT_UINT8_TO_UINT16(arr_Commun[2],arr_Commun[3]));
//					pcmd_Commun->aim=0;
//					pcmd_Commun->cmd=ParaCmd_GetDataMore;

//					pta_Commun->Concierge.GiftList[id_Commun].ExternUnit.Data=(MODBUS_CONNECT_UINT8_TO_UINT16(arr_Commun[4],arr_Commun[5]));
//					pta_Commun->Concierge.GiftList[id_Commun].ExternUnit.pData=(void *)(qc_Commun->pTxTail);

//					break;

//				case 4:
//					*(qc_Commun->pTxTail)++=4;
//					*(qc_Commun->pTxTail)++=(MODBUS_CONNECT_UINT8_TO_UINT16(arr_Commun[4],arr_Commun[5]))*MIAO_PARA_NORMAL_LENGTH;

//					pcmd_Commun=&(pta_Commun->Concierge.GiftList[id_Commun].PrivateUnit.ParaCmd);

//					pcmd_Commun->ListID=0;
//					pcmd_Commun->ID=(MODBUS_CONNECT_UINT8_TO_UINT16(arr_Commun[2],arr_Commun[3]));
//					pcmd_Commun->aim=0;
//					pcmd_Commun->cmd=ParaCmd_GetDataMore;

//					pta_Commun->Concierge.GiftList[id_Commun].ExternUnit.Data=(MODBUS_CONNECT_UINT8_TO_UINT16(arr_Commun[4],arr_Commun[5]));
//					pta_Commun->Concierge.GiftList[id_Commun].ExternUnit.pData=(void *)(qc_Commun->pTxTail);

//					break;

//				case 6:

//					pcmd_Commun=&(pta_Commun->Concierge.GiftList[id_Commun].PrivateUnit.ParaCmd);

//					pcmd_Commun->ListID=1;
//					pcmd_Commun->ID=(MODBUS_CONNECT_UINT8_TO_UINT16(arr_Commun[2],arr_Commun[3]));
//					pcmd_Commun->aim=0;
//					pcmd_Commun->cmd=ParaCmd_SetNormal;


//					arr_Commun+=1;
//					n_Commun=5;
//					MIAO_TOOL_MEMCPY(qc_Commun->pTxTail,arr_Commun,n_Commun);

//					arr_Commun=pta_Commun->Concierge.GiftList[id_Commun].PrivateUnit.ParaCmd.data.para_arr;
//#if MIAO_PARA_NORMAL_LENGTH >= 4
//					*arr_Commun++ = *(qc_Commun->pTxTail-4);
//#endif
//#if MIAO_PARA_NORMAL_LENGTH >= 3
//					*arr_Commun++ = *(qc_Commun->pTxTail-3);
//#endif
//#if MIAO_PARA_NORMAL_LENGTH >= 2
//					*arr_Commun++ = *(qc_Commun->pTxTail-2);
//#endif
//#if MIAO_PARA_NORMAL_LENGTH >= 1
//					*arr_Commun++ = *(qc_Commun->pTxTail-1);
//#endif
//					break;
//
//				default:
//					break;
//				}
//
//				MiaoTaskGiftTransmit(pta_Commun,pta_Commun->pOS->pTaskList[1],id_Commun,&(pta_Commun->Concierge.GiftList[id_Commun]),GiftPostmanStatus_Return);
//		//		qc_Commun->pTxTail+=*(qc_Commun->pTxTail - 1);
//
//		//		pta_Commun->pOS->pta_CommunskList[1]->TaskStatus=pta_Commun->pOS->pta_CommunskList[1]->TaskStatus==
//		//		MiaoTaskStatus_Wait_ForGiftReceive?MiaoTaskStatus_Ready:MiaoTaskStatus_Wait_ForGiftReceive;
//
//				MiaoGiftTransmitWait(pta_Commun);
//			}
//			else;
//

//			qc_Commun->pTxTail+=qc_Commun->pTxHead[2];
//			qc_Commun->TxReadyFun(qc_Commun);
//			qc_Commun->TxLength+=qc_Commun->pTxHead[2]+3+2;
//			MiaoCommunTransmit(qc_Commun);
//
//			pgr_Commun->pHead->Status=pgr_Commun->pHead->Method;
//
//		}while(pgr_Commun->MoveNextFun(pgr_Commun));

//		//qc_Commun->pTxTail=qc_Commun->TxList;
//	}

}


void MiaoModBusCallBack(MiaoCore_t *pOS,lw_u8 ComID,lw_u8 *pdata,lw_u16 len)
{
//
//	MiaoGiftPostman_t *p;
//	MiaoCommun_t *q=& (pOS->Comm[ComID]);
//	lw_8 inID;
//
//	MIAO_assert(!pOS);
//	MIAO_assert(!pdata);
//
//	q->ComRX.AppendFun( & (q->ComRX),pdata,sizeof(lw_u8)*len);
//
//	if(q->isReadyFun(q))
//	{
//		q->ComRX.MDataLockFlag=1;
//		inID=MiaoGiftTransmitIdleIndex(&q->giftT);
//		if(inID!=(lw_8)-1)
//		{
//			p=&(q->giftT.PostmanList[inID]);//取得快递员
//
//			q->GiftList[inID].PrivateUnit.ComCmd.ComID=ComID;
//
//			memcpy(q->GiftList[inID].PrivateUnit.ComCmd.RxD,q->RxList,sizeof(lw_u8)*MIAO_COMMUN_RX_LENGTH);
//
//			p->pGU=&(q->GiftList[inID]);//带上礼物
//
//			p->Method=GiftPostmanStatus_Received;//告诉他模式
//
//			MiaoGiftReceiveAppend(& (pOS->pTaskList[2]->Concierge.giftR) ,p);
//		}
//		q->ComRX.MDataLockFlag=0;
//
//	}
}


/*(Ver.=0.96)
lw_8 MiaoModBusIsReady(void *pcom)
{


	uint32_t crcnum=MODBUS_CONNECT_UINT8_TO_UINT16(pcom->RxList[MIAO_COMMUN_RX_LENGTH-2],pcom->RxList[MIAO_COMMUN_RX_LENGTH-1]);

	MIAO_assert(!pcom);
	if(CRC16(pcom->RxList,MIAO_COMMUN_RX_LENGTH-2)==crcnum)
	{
		return 1;
	}

	return 0;

}

void MiaoModBusTxReady(void *pcom)
{

	lw_u16 crcnum=CRC16(pcom->pTxHead,pcom->pTxHead[2]+3);
	*(pcom->pTxTail)++=(crcnum>>8)&0x00ff;
	*(pcom->pTxTail)++=(crcnum>>0)&0x00ff;

}

*/
const lw_u8 auchCRCHi[] = {
    
} ;

// CRC 低位字节值表
const lw_u8 auchCRCLo[] = {
    
};


lw_u16 CRC16(lw_u8 *puchMsg, lw_u16 usDataLen) 	           // CRC 生成函数
{
    lw_u8 uchCRCHi = 0xFF ;                                // 高 CRC 字节初始化
    lw_u8 uchCRCLo = 0xFF ;                                // 低 CRC 字节初始化
    uint32_t uIndex ;                                        // CRC 循环中的索引
    while (usDataLen--)                                    // 传输消息缓冲区
    {
        uIndex = uchCRCHi ^ *puchMsg++ ;                   // 计算CRC
        uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex] ;
        uchCRCLo = auchCRCLo[uIndex] ;
    }

    return (uchCRCHi << 8 | uchCRCLo) ;                    // 返回 CRC 校验数值
}

#endif

