...
void MiaoTaskDispatch(MiaoCore_t *_pOS)
{
    ...
    {
        while(pOS->pCurrentTask->TaskStatus != MiaoTaskStatus_Ready);//直接等待任务进入准备状态
...
#if !MIAO_SIMPLE_MODE
                if(!_MIAO_SAVE(pOS->DispatchBreakPoint))//等待任务跳回来
                {
    ...
    if(pOS->_flag_BK)//屏蔽最低优先级--空闲任务 查看任务是否准备完毕
    {
        ...
        if (pOS->TaskRunningNum > __MIAO_LOG2_TASK_NUM  )//如果任务数量比较多 说明二分法搜索时间复杂度较低
        {

            //二分法
            for (pOS->n2_Dispatch = 0; pOS->n2_Dispatch < __MIAO_LOG2_TASK_NUM - 2; pOS->n2_Dispatch++)//二分法折半到4 直接switch case
            {
                /*(Ver.=0.96)
                	TASK_NUM == 8 为例
        ...
        //pOS->pCurrentTask->TaskStatus=MiaoTaskStatus_Running;//更改任务状态
				
        _MIAO_LOAD(pOS->pCurrentTask->BreakPoint[MIAO_TASK_BREAKPOINT_DEEP - 1], pOS->LastTaskIndex,pOS->pCurrentTask);//任务跳转
...
void MiaoClockCallBack(MiaoCore_t *pOS)
{
	...
	else
	{
		...
		if (pOS->CurrentTaskIndex==MIAO_TASK_NUM_MAX-1)//如果是空闲任务
		{
    ...
    for(; n<pOS->TaskRunningNum-1; n++) //循环任务列表
    {
        ...
        case MiaoTaskStatus_Wait_ForSleep://等待睡眠状态
            if(!(--(pta->SleepCount)))//睡眠寄存器-- 如果为0，说明的是时间到了
            {
                MiaoCoreTaskIsReady(pOS,pta,tindex);//转换任务状态 为准备态
            }//end if
            break;
...
#if MIAO_SIMPLE_LEVEL < 3
        case MiaoTaskStatus_Wait_ForGiftReceive://等待接收状态
...
void MiaoCoreInitial(MiaoCore_t *pOS)
{
    ...
    MIAO_assert(!pOS);
    memset(pOS,0,sizeof(MiaoCore_t));//清空内容
    memset(pOS->TaskRunningList,MIAO_TASK_NUM_MAX+1,sizeof(pOS->TaskRunningList));//执行表初始化
...
//	}
    ...
    MiaoTaskAppend(pOS, &(pOS->MiaoInlineTaskBoring));//空闲任务追加
    ...
    /*(Ver.=0.96)
    	...
    	MiaoTaskAppend(pOS,&MiaoInlineTaskParameter);//参数管理任务
    */
/*(Ver.=0.96)
...
*/
#if MIAO_TASK_COMMUN_ENABLE //如果使能通信任务
//	for(i=0;i<MIAO_CORE_COMMUN_LIST_LENGTH;i++)//循环通信控制器列表
//	{
//		//指针初始化
...
//		//操作系统指针
//		pOS->Comm[i].pOS=pOS;
//		pOS->Comm[i].Address=MIAO_COMMUN_ADDRESS;//地址
//
...
//			);
//	MiaoTaskAppend(pOS,&MiaoInlineTaskCommun);//系统通信任务
...
//逐渐放弃 鸡肋
    ...
    MiaoTaskAppend(pOS,&MiaoInlineTaskShow);//系统显示任务
...
MiaoCore_t *MiaoTaskAppend(MiaoCore_t *pOS,MiaoTask_t *pta)
{
    ...
    {
        pOS->CoreError=MiaoCoreError_PriorityRedefine;//内核异常置起
        return pOS;//返回
    }
...
MiaoCore_t* MiaoParaAppend(MiaoCore_t *pOS,lw_u8 num,struct MIAO_PARAMETER_STRUCT *p)
{
	...
	num= num % MIAO_CORE_PARA_CTRL_NUM;//% 保证列表编号不会越界
	pPC = & (pOS->ParaCtrlList[num]);// 取得系统参数控制列表的指针
	if (pPC->ParaLength >= MIAO_PARA_LIST_LENGRH)//如果参数数量已经超过列表总长度
	{
		pOS->CoreError=MiaoCoreError_ParaListLengthOver;//设置错误标志
		return pOS;
	}
...
MiaoCore_t* MiaoParaAppend(MiaoCore_t *pOS,lw_u8 PageId,MiaoParameter_t *pP)
{
    ...
    if (pPLC->ParaLength >= MIAO_PARA_LIST_LENGRH)//如果参数数量已经超过列表总长度
    {
        pOS->CoreError=MiaoCoreError_ParaListLengthOver;//设置错误标志
        return pOS;
    }
...
void MiaoCoreStart(MiaoCore_t *pOS)
{
    ...
    pOS->CoreStatus=MiaoCoreStatus_Running;//任务内核转换为运行态
#if !(MIAO_SIMPLE_MODE)
    MiaoTaskDispatch(pOS);//执行任务调度
#else
