...
)
{
	...
	//分配节点内存
	node = pmem->Malloc(pmem, sizeof(MiaoToolNNLayerFullCon_t));

	memset(node, 0, sizeof(MiaoToolNNLayerFullCon_t));

	node->basic.NN_Kind=MiaoToolNNSubLayerKind_FullCon;
	...
	MiaoToolMatrixInitial(&(node->basic.in), InCol, max_mini_batch, pin);
	...
	MiaoToolMatrixInitial(&(node->basic.out), OutCol, max_mini_batch, pout);
#else
...
#error"NN data frame is unknown"
	...
	//成员函数
	node->basic.Forward = MiaoToolBPnnLayerFullConForward;
	node->basic.Backward = MiaoToolBPnnLayerFullConBackward;
...
lw_ptr MiaoToolBPnnLayerFullConForward(struct MIAO_TOOL_NN_SUB_LAYER_BASIC_STRUCT *pNNSLB, lw_ptr mini_b_num)
{
	...
	register MiaoToolNNLayerFullCon_t *pfc;
	MIAO_assert(!(pNNSLB));
...
lw_ptr MiaoToolBPnnLayerFullConBackward(struct MIAO_TOOL_NN_SUB_LAYER_BASIC_STRUCT *pNNSLB, lw_ptr mini_b_num)
{
	...
	//	//直接覆盖
	//	//dw=in'*out
	//	pMiaoToolMatrixTurnMul(&(pfc->DeltW), &(pNNSLB->in), &(pNNSLB->out), 2);
	//	//db = sum(out, 1);按列求和
	//	//_out_b = pNNSLB->out.a;
	//	//_db = pfc->DeltB.a;
	//	//row = pNNSLB->out.col;
	...
	//	//xd = out*w';
	//	pMiaoToolMatrixTurnMul(&(pNNSLB->in), &(pNNSLB->out), &(pfc->Weight), 1);
	//}
	//else
	//{
	//	//采用叠加方法
	//	//dw=in'*out
	//	pMiaoToolMatrixTurnMul(&(pfc->DeltW), &(pNNSLB->in), &(pNNSLB->out), 6);
	//	//db = sum(out, 1);按列求和
	//	_out_b = pNNSLB->out.a;
	//	_db = pfc->DeltB.a;
	//	row = pNNSLB->out.col;
	...
	if (pNNSLB->pNN->Train.Flag.NeedTrain== MiaoToolNNNeedTrain_Need)
	{
		...
		pMiaoToolMatrixTurnMul(&(pfc->DeltW), &(pNNSLB->in), &(pNNSLB->out), 6);
		//db = sum(out, 1);按列求和
		pMiaoToolMatrixSum(&(pfc->DeltB), &(pNNSLB->out), 0x12);
	}
	...
	if (pNNSLB->pNN->Train._batch_stream_count <= 1)
	{
		//直接覆盖
		//dw=out*in'
		pMiaoToolMatrixTurnMul(&(pfc->DeltW), &(pNNSLB->out), &(pNNSLB->in), 0x01);
		...
		//xd = out'*w;
		pMiaoToolMatrixTurnMul(&(pNNSLB->in), &(pfc->Weight), &(pNNSLB->out), 0x02);
	}
	else
	{
		//采用叠加方法
		//dw=out*in'
		pMiaoToolMatrixTurnMul(&(pfc->DeltW), &(pNNSLB->out), &(pNNSLB->in), 0x5);
		...
		//xd = out'*w;
		pMiaoToolMatrixTurnMul(&(pNNSLB->in), &(pfc->Weight), &(pNNSLB->out), 0x2);
	}
...
{
    register uint32_t i, all_batch_count;
	...
	l2regu_k = _ParaFint(1) - _ParaDiv(_ParaMul(pNNSLB->nl, pNNSLB->LamdaL2), _ParaFint(all_batch_count));
	...
	case MiaoToolNNOptim_GradDesc:
		k = _ParaDiv(_ParaMul(pBPnn->_nl_factor, pNNSLB->nl), _ParaFint(all_batch_count));
		...
		if (!(pBPnn->Train.Flag.EnableDeltLimit))
		{
			...
			{
				*_mw = _ParaAdd(_ParaMul(*_mw, l2regu_k), _ParaMul(k, *_mwd));
				*_mb = _ParaAdd(*_mb, _ParaMul(k, *_mbd));
				*_mwd = _ParaFint(0);
				*_mbd = _ParaFint(0);
			}
		...
		else
		{
			...
			for (i = 0; i < pfc->Bias.length; i++, _mw++, _mwd++, _mb++, _mbd++)
			{
				temp = _ParaMul(k, *_mwd);
				LIMIT_DELT(temp, DeltMax, DeltMin);
				*_mw = _ParaAdd(_ParaMul(*_mw, l2regu_k), temp);
				temp = _ParaMul(k, *_mbd);
				LIMIT_DELT(temp, DeltMax, DeltMin);
	...
	case MiaoToolNNOptim_M:
		...
		k = _ParaMul(pBPnn->_nl_factor, pNNSLB->nl);
		...
		if (!(pBPnn->Train.Flag.EnableDeltLimit))
		{
			...
			{
				//计算出的梯度要先除以batch
				wbuf = _ParaMul(*_mwd, _div_m);
				bbuf = _ParaMul(*_mbd, _div_m);

				//动量法 v=0.9v+0.1d
				*_vw = _ParaMul(*_vw, b1) + _ParaMul(wbuf, nb1);
				*_vb = _ParaMul(*_vb, b1) + _ParaMul(bbuf, nb1);

				//w=w+v
				*_mw = _ParaAdd(_ParaMul(*_mw, l2regu_k), _ParaMul(k, *_vw));
				*_mb = _ParaAdd(*_mb, _ParaMul(k, *_vb));
				*_mwd = _ParaFint(0);
				*_mbd = _ParaFint(0);
			}
			i = pfc->Weight.length - pfc->Bias.length;
		...
		else
		{
			...
			for (i = 0; i < pfc->Bias.length; i++, _mw++, _mwd++, _vw++, _mb++, _mbd++, _vb++)
			{
				...
				*_vb = _ParaMul(*_vb, b1) + _ParaMul(bbuf, nb1);
				...
				//w=w+v
				temp = _ParaMul(k, *_vw);LIMIT_DELT(temp, DeltMax, DeltMin);
				*_mw = _ParaAdd(_ParaMul(*_mw, l2regu_k),temp);

				temp = _ParaMul(k, *_vb);LIMIT_DELT(temp, DeltMax, DeltMin);
				*_mb = _ParaAdd(*_mb, temp);
				*_mwd = _ParaFint(0);
				*_mbd = _ParaFint(0);
			}
	...
	case MiaoToolNNOptim_RMSp:
		...
		if (!(pBPnn->Train.Flag.EnableDeltLimit))
		{
			...
			for (i = 0; i < pfc->Bias.length; i++, _mw++, _mwd++, _uw++, _mb++, _mbd++, _ub++)
			{
				...
				//RMS法u=0.9u+0.1d*d
				*_uw = _ParaMul(*_uw, b2) + _ParaMul(_ParaMul(wbuf, wbuf), nb2);
				*_ub = _ParaMul(*_ub, b2) + _ParaMul(_ParaMul(bbuf, bbuf), nb2);

				temp = _ParaMul(k, _ParaDiv(wbuf, (e + _ParaSqrt(*_uw))));LIMIT_DELT(temp, DeltMax, DeltMin);
				*_mw = _ParaAdd(_ParaMul(*_mw, l2regu_k), temp);

				temp = _ParaMul(k, _ParaDiv(bbuf, (e + _ParaSqrt(*_ub))));LIMIT_DELT(temp, DeltMax, DeltMin);
				*_mb = _ParaAdd(*_mb, temp);
	...
	case MiaoToolNNOptim_Adam:
		...
		if (!(pBPnn->Train.Flag.EnableDeltLimit))
		{
			...
			for (i = 0; i < pfc->Bias.length; i++, _mw++, _mwd++, _uw++, _vw++, _mb++, _mbd++, _ub++, _vb++)
			{
				...
				*_mw = _ParaAdd(_ParaMul(*_mw, l2regu_k), _ParaMul(k, _ParaDiv(vwbuf, (e + _ParaSqrt(uwbuf)))));
				*_mb = _ParaAdd(*_mb, _ParaMul(k, _ParaDiv(vbbuf, (e + _ParaSqrt(ubbuf)))));
		...
		else
		{
			...
			for (i = 0; i < pfc->Bias.length; i++, _mw++, _mwd++, _uw++, _vw++, _mb++, _mbd++, _ub++, _vb++)
			{
				...
				*_ub = _ParaMul(*_ub, b2) + _ParaMul(_ParaMul(bbuf, bbuf), nb2);
				...
				vbbuf = _ParaDiv(*_vb, b1t);
				ubbuf = _ParaDiv(*_ub, b2t);
				temp = _ParaMul(k, _ParaDiv(vwbuf, (e + _ParaSqrt(uwbuf))));LIMIT_DELT(temp, DeltMax, DeltMin);
				*_mw = _ParaAdd(_ParaMul(*_mw, l2regu_k), temp);

				temp = _ParaMul(k, _ParaDiv(vbbuf, (e + _ParaSqrt(ubbuf))));LIMIT_DELT(temp, DeltMax, DeltMin);
				*_mb = _ParaAdd(*_mb, temp);
...
lw_ptr MiaoToolBPnnLayerFullConNNmalloc(struct MIAO_TOOL_NN_SUB_LAYER_BASIC_STRUCT *pNNSLB, void *pmemForward, void *pmemBackward)
{
	...
	if (pmemForward)
	{
		MiaoToolMatrixInitial(&(pfc->Weight), pfc->Weight.row, pfc->Weight.col, pmemForward);
		MiaoToolMatrixInitial(&(pfc->Bias), pfc->Bias.row, pfc->Bias.col, (ParaType *)(pmemForward)+pfc->Weight.length);
	}
	//后向传播内存空间 D*n+W*n n为多余空间
	if (pmemBackward)
	{
		MiaoToolMatrixInitial(&(pfc->DeltW), pfc->DeltW.row, pfc->DeltW.col, pmemBackward);
		MiaoToolMatrixInitial(&(pfc->DeltB), pfc->DeltB.row, pfc->DeltB.col, (ParaType *)(pmemBackward)+(pfc->DeltW.length) * (pNNSLB->pNN->Train._MemAllocCoff));
	}
...
lw_ptr MiaoToolBPnnLayerFullConClear(struct MIAO_TOOL_NN_SUB_LAYER_BASIC_STRUCT *pNNSLB)
{
	...
	MiaoToolBPnnLayerClearDefault(pNNSLB);//调用基类函数
...
lw_ptr MiaoToolBPnnLayerFullConDataInit(struct MIAO_TOOL_NN_SUB_LAYER_BASIC_STRUCT *pNNSLB)
{
	...
	register MiaoNNinitFunType pfun;
	register void *pr;
...
{
	MiaoToolNNLayerFullCon_t *pfc1, *pfc2;
	MIAO_assert(!(pNNSLB1));
	MIAO_assert(!(pNNSLB2));
	
	pfc1 = (MiaoToolNNLayerFullCon_t *)pNNSLB1;
	pfc2 = (MiaoToolNNLayerFullCon_t *)pNNSLB2;
...
)
{
	...
	if (pTemplet)
	{
		...
		pnode1 = MiaoToolNNLayerFullConNew
		(
			pin, NULL,
			in_col, out_col,
		...
		pfc = (MiaoToolNNLayerFullCon_t *)pnode1;
	}
	else//采用不带激活函数的方式
	{
		_NN_GET_IN(pBPnn, pin);
...
)
{
	...
	//分配节点内存
	node = pmem->Calloc(pmem, sizeof(MiaoToolNNLayerFullCon_t));
	node->basic.NN_Kind = MiaoToolNNSubLayerKind_Shift;
	...
	//成员函数
	node->basic.Forward = MiaoToolBPnnLayerShiftForward;
	node->basic.Backward = MiaoToolBPnnLayerShiftBackward;
...
lw_ptr MiaoToolBPnnLayerShiftBackward(struct MIAO_TOOL_NN_SUB_LAYER_BASIC_STRUCT *pNNSLB, lw_ptr mini_b_num)
{
	...
	if (pNNSLB->pNN->Train.Flag.NeedTrain == MiaoToolNNNeedTrain_Need)
	{
		//****只完成了权重为列向量的反向传递****
		//dw=sum(in.*out,2)
		pMiaoToolMatrixDotSum(&(pfc->DeltW), &(pNNSLB->in), &(pNNSLB->out), 0x12);
		//db = sum(out, 1);按列求和
		pMiaoToolMatrixSum(&(pfc->DeltB), &(pNNSLB->out), 0x12);
	}
	//xd = out.*remat(we);
	pMiaoToolMatrixVectShift(&(pNNSLB->in), &(pNNSLB->out), &(pfc->Weight), NULL);
