...
ParaType MiaoToolPidCtrlCal(MiaoToolPidCtrl_t *p)
{
    ...
    p->K_up = _ParaMul(p->K_e,p->Para_Kp.data.RealPara.Parameter);     //计算比例的输出
    ...
    p->K_ui+=_ParaAdd(_ParaMul(p->Para_Ki.data.RealPara.Parameter,p->K_e), _ParaMul(p->Para_Kc.data.RealPara.Parameter,_ParaSub(p->O_uout,p->K_upresat))); //计算积分项的输出
    p->K_ud = _ParaMul(p->Para_Kd.data.RealPara.Parameter,_ParaSub(p->K_e,p->K_eup));   //计算微分相输出
    p->K_eup=	p->K_e;
    return p->O_uout;
}
