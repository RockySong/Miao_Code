...
static const lw_u8 _kind_info[][4] = { {'c','c',sizeof(lw_u8),0},{ 'i','d',sizeof(lw_u16),0 },{ 'l','d',sizeof(uint32_t),0 },{ 'f','f',sizeof(lw_sf),0 },{ 'F','s',0,0 } };
...
{
	memset(pSD,0,sizeof(struct MIAO_STRING_DEBUG_STRUCT));
	
#if MIAO_STR_DBG_SIMPLE_MODE==0
	MiaoMemoryMapInital(&(pSD->DebugMem), pSD->DebugHeap, MIAO_STR_DBG_MEM_LEN, MIAO_STR_DBG_MEM_SIZE, sizeof(pSD->DebugHeap));
#endif
	
#if MIAO_STR_DBG_SIMPLE_MODE==0 || MIAO_STR_DBG_SIMPLE_MODE==1 	
	MiaoToolHashInitial(&(pSD->DebugHash), MIAO_STR_DBG_HASH_LEN, HashCalStringBKDR, HashCmpString);
#endif
...
uint32_t _StringDebugMemCalSq(lw_u8 *data, lw_u8 *info, lw_u8 *pNum)
{
	...
	*pNum = _len;
	return (uint32_t)_len * (uint32_t)info[2];
}
...
uint32_t _StringDebugMemLengthPl(lw_u8 *data, lw_u8 *info,lw_sf *pstart, lw_sf *pdelt,lw_u8 *pNum)
{
	...
	num = (uint32_t)((_end - _start) / _delt) & 0xff;
	...
	*pNum = num+1;
	...
	return (uint32_t)(num+1) * (uint32_t)info[2];
}

uint32_t _StringDebugMemLengthBrace(lw_u8 *data, lw_u8 *info,lw_u8 *pNum)
{
	lw_u8 _num;
	_num = _StringDebugFindCharNum(data, ',');

	*pNum = _num+1;
	return (uint32_t)(_num+1) * (uint32_t)info[2];
}
...
lw_8 * StringDebug(MiaoToolHash_t *ph, MiaoMemoryBasic_t *pmem, lw_8 *instr, lw_8 *outstr, lw_8 *data1, lw_8 *data2)
{
	...
	case '#':
		...
		{
			pdln = pmem->Malloc(pmem, sizeof(MiaoToolDListNode_t));
...
lw_8 * StringDebug(MiaoStrDbg_t *pSD, lw_8 *instr, lw_8 *outstr, lw_8 *data1, lw_8 *data2)
{
	...
	case '?':
		...
		{
			case 'c':
				MiaoToolString_itoa(*((lw_8 *)pseldata),outstr,10);
				break;
			
			case 'i':
				MiaoToolString_itoa(*((lw_16 *)pseldata),outstr,10);
				break;
			case 'l':
				MiaoToolString_itoa(*((lw_32 *)pseldata),outstr,10);
				break;
...
#endif //MIAO_STR_DBG_SIMPLE_MODE==0
...
lw_u8 * MiaoStrDbgFun(struct MIAO_CORE_STRUCT *pOS, lw_u8 *instr)
{
	...
	MiaoStrDbgRun(&(pOS->StrDbg),instr);
